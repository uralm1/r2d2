...
sub register {
  ...
  $app->helper(fw_add_replace => sub {
    ...
    for (@filter_content) {
      ...
      #
      # 450
      #(1)-A pipe_in_inet_clients -d 192.168.34.23 -j ACCEPT
      #(2)-A pipe_out_inet_clients -s 192.168.34.23 -m mac --mac-source 11:22:33:44:55:66 -j ACCEPT
      if ($skip > 0) {
        if (/^-A\ /x) {
          ...
          # here actual replace
          print $fh "# $v->{id}\n";
          print $fh "-A $client_in_chain -d $v->{ip} -j $v->{defjump}\n";
          my $m = ($v->{mac}) ? "-m mac --mac-source $v->{mac} " : '';
          print $fh "-A $client_out_chain -s $v->{ip} ${m}-j $v->{defjump}\n";
          $skip = 0;
        }
      }
      if (/^\#\ \Q$v->{id}\E$/x) {
    ...
    for (@mangle_content) {
      ...
      #:pipe_out_inet_clients - [0:0]
      # 450
      #(1)-A pipe_in_inet_clients -d 192.168.34.23
      #(2)-A pipe_out_inet_clients -s 192.168.34.23
      if ($skip > 0) {
        if (/^-A\ /x) {
          #say "Skipped line ($skip) $_";
          $skip++;
          ...
          # here actual replace
          print $fh "# $v->{id}\n";
          print $fh "-A $client_in_chain -d $v->{ip}\n";
          print $fh "-A $client_out_chain -s $v->{ip}\n";
          $skip = 0;
        }
      }
      if (/^\#\ \Q$v->{id}\E$/x) {
