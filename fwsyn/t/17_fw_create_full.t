...
my $t = Test::Mojo->new('Fwsyn', { firewall_file => $test_f->to_string,
  client_in_chain=>'in_test', client_out_chain=>'out_test',
  my_profiles => ['plk'],
  worker_db_file => '/tmp/test$$.dat',
  #old_iptables_compatibility => 0,
});
my $j = [
...
$t = Test::Mojo->new('Fwsyn', { firewall_file => $test_f->to_string,
  client_in_chain=>'in_test', client_out_chain=>'out_test',
  my_profiles => ['plk'],
  worker_db_file => '/tmp/test$$.dat',
  old_iptables_compatibility => 1,
});
is($t->app->fw_create_full($j), 1, 'fw_create_full2');
...
@@ result2
# WARNING: this is autogenerated file, don't run or change it!
# Compatibility mode is switched on, you must flush user chains prior calling iptables-restore.
