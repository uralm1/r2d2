...
sub register {
  ...
  my $_print_block = sub {
    ...
    print $fh "$tc_path qdisc add dev \$INTR_IF parent 1:$rule_id handle $rule_id: pfifo limit 150\n";
    print $fh "$tc_path filter add dev \$INTR_IF parent 1:0 protocol ip pref 10 u32 match ip dst $dh->{ip} flowid 1:$rule_id\n";
    print $fh "$tc_path filter add dev \$INTR_IF parent 1:0 protocol ip pref 4 u32 match ip dst $dh->{ip} match ip src 10.15.0.1 match ip sport 3128 0xffff flowid 1:$rule_id\n";
    print $fh "$tc_path class add dev \$EXTR_IF parent 1:10 classid 1:$rule_id htb $dh->{speed_out}\n";
    print $fh "$tc_path qdisc add dev \$EXTR_IF parent 1:$rule_id handle $rule_id: pfifo limit 120\n";
    print $fh "$tc_path filter add dev \$EXTR_IF parent 1:0 protocol ip pref 10 u32 match ip src $dh->{ip} flowid 1:$rule_id\n";
    print $fh "$tc_path filter add dev \$EXTR_IF parent 1:0 protocol ip pref 4 u32 match ip dst $dh->{ip} match ip src 192.168.12.1 match ip sport 3128 0xffff flowid 1:$rule_id\n";
    print $fh "\n";
  };
  ...
  $app->helper(tc_add_replace => sub {
    ...
    for (@content) {
      ...
      #(2)/usr/sbin/tc qdisc add dev $INTR_IF parent 1:300 handle 300: pfifo limit 150
      #(3)/usr/sbin/tc filter add dev $INTR_IF parent 1:0 protocol ip pref 10 u32 match ip dst 192.168.34.23 flowid 1:300
      #(4)/usr/sbin/tc filter add dev $INTR_IF parent 1:0 protocol ip pref 4 u32 match ip dst 192.168.34.23 match ip src 10.15.0.1 match ip sport 3128 0xffff flowid 1:300
      #(5)/usr/sbin/tc class add dev $EXTR_IF parent 1:10 classid 1:300 htb quantum 6400 rate 2mbit prio 5
      #(6)/usr/sbin/tc qdisc add dev $EXTR_IF parent 1:300 handle 300: pfifo limit 120
      #(7)/usr/sbin/tc filter add dev $EXTR_IF parent 1:0 protocol ip pref 10 u32 match ip src 192.168.34.23 flowid 1:300
      #(8)/usr/sbin/tc filter add dev $EXTR_IF parent 1:0 protocol ip pref 4 u32 match ip dst 192.168.34.23 match ip src 192.168.12.1 match ip sport 3128 0xffff flowid 1:300
      #(9)
      if ($skip > 0) {
        if (/^\Q$tc_path\E/x) {
